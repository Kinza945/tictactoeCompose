package com.kynzai.tictactoe.ui.viewmodel
...
import com.kynzai.tictactoe.algorithm.Algorithm
import com.kynzai.tictactoe.data.*
import com.kynzai.tictactoe.model.to2DArray
...
class GameViewModel @Inject constructor(
	...
	val board = mutableStateListOf<List<PointAction>>()
	...
	private val gameEngine = Algorithm(
		playerOne = playerOne,
		playerTwo = playerTwo
	)
	...
	init {
		gameEngine.setListener(object : Algorithm.Listener {
			override fun onWin(type: WinType) {
				winner = type

				when {
					playerOne.pointType == PointAction.O && winner == WinType.O -> {
						playerOne = playerOne.copy(win = playerOne.win + 1)
					}
					playerOne.pointType == PointAction.X && winner == WinType.X -> {
						playerOne = playerOne.copy(win = playerOne.win + 1)
					}
					playerTwo.pointType == PointAction.O && winner == WinType.O -> {
						playerTwo = playerTwo.copy(win = playerTwo.win + 1)
					}
					playerTwo.pointType == PointAction.X && winner == WinType.X -> {
						playerTwo = playerTwo.copy(win = playerTwo.win + 1)
					}
					winner == WinType.Tie -> {
						draw += 1
